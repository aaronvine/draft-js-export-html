import { Entity, ContentState, ContentBlock, EntityInstance } from 'draft-js';

type StringMap = {[key: string]: string};
type AttrMap = {[key: string]: StringMap};
type CharacterMetaList = any;

const getEntityRanges = require('./getEntityRanges');
...
function getWrapperTag(blockType: string): any {
	switch (blockType) {
		case BLOCK_TYPE.UNORDERED_LIST_ITEM:
			return 'ul';
		case BLOCK_TYPE.ORDERED_LIST_ITEM:
...
// Start of MarkupGenerator class
...
class MarkupGenerator {
	...
	renderBlockContent(block: ContentBlock): string {
		...
		return entityPieces.map(([entityKey, stylePieces]) => {
			...
			let entity : any = entityKey ? Entity.get(entityKey) : null;
			...
			if (entityType != null && entityType === ENTITY_TYPE.LINK) {
				...
				return '<span data-mentioned-user-id="' + entity.get('data').mention.get('userId') + '">' + content + '</span>';
			} else {
				return content;
			}
		}).join('');
